...
actor TimelineStoreActor {
    ...
    func saveTimeline(_ items: [TimelineItem]) async throws {
        let data = try JSONEncoder().encode(items)
        try data.write(to: documentsURL.appendingPathComponent("timeline.json"))
    }
    
    func loadTimeline() async -> [TimelineItem] {
        let url = documentsURL.appendingPathComponent("timeline.json")
        guard let data = try? Data(contentsOf: url) else { return [] }
        return (try? JSONDecoder().decode([TimelineItem].self, from: data)) ?? []
    }
    
    func generateTimeline(from weddingDate: Date) -> [TimelineItem] {
