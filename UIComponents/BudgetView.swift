...
struct BudgetView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    BudgetHeaderReplacement(categories: $categories, totalBudget: totalBudget, totalSpent: totalSpent, remaining: remaining)
                        .padding(.horizontal)
            ...
            .alert("Add Expense", isPresented: $showingExpenseAlert) {
                ...
                Button("Add") {
                    if let amount = Double(expenseAmount), let index = categories.firstIndex(where: { $0.id == selectedCategory?.id }) {
